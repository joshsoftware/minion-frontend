...
import UserDashboardServerTelemetryPanel from '../UserDashboardServerTelemetryPanel'
...
const UserDashboardServerTelemetryCard = props => {
  ...
  // There is a standard set of telemetry query keys for which the UI has pretty names.
  const telemetryTypeNames = {
    load_avg: 'System Load Average',
    mem_used_kb: 'Memory Usage (KB)',
    memfree_kb: 'Free RAM (KB)',
    cached_kb: 'Cached Data (KB)',
    buffers_kb: 'Buffer Size (KB)',
    memtotal_kb: 'Total RAM (KB)'
  }
  ...
  const [primaryDataKeys, setPrimaryDataKeys] = useState([])
  useEffect(() => {
    const getPDK = async () => {
      const pdk = await TelemetryService.get_primary_data_keys(
        props.selectedServers
      )
      setPrimaryDataKeys(pdk.data)
    }
            ...
            <Box className='rounded card-body mb-4'>
              ...
              <div className='card-text mb-4'>
                <TextField
                  ...
                  select
                  label='Telemetry Selection Criteria'
                  value={criteria}
            ...
            <div className='card-header py-3'>
              <div className='card-header--actions'>
                <Button
                  ...
                  <span className='btn-wrapper--label'>Close Telemetry</span>
                </Button>
              </div>
            </div>
          </Card>
            ...
            {props.selectedServers &&
              ...
              props.selectedServers.map((server, index) => (
                <TabPanel key={server} value={value} index={index}>
                  <UserDashboardServerTelemetryPanel
                    server={server}
                    criteria={appliedCriteria}
                    criteriaTrigger={criteriaTrigger}
                    typeLabels={telemetryTypeNames}
                  />
                </TabPanel>
              ))}
          </div>
...
export default UserDashboardServerTelemetryCard
